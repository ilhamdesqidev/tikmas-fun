    ...
    public function processCheckout(Request $request, $id)
    {
        ...
        if ($promo->quota) {
            ...
            if ($request->ticket_quantity > $remainingQuota) {
                return back()
                    ->with('error', "Maaf, kuota hanya tersisa {$remainingQuota} tiket.")
                    ->withInput();
            }
            ...
            if ($remainingQuota <= 0) {
                return back()
                    ->with('error', 'Maaf, kuota promo sudah habis.')
                    ->withInput();
            }
        }
        ...
        if ($visitDate->lt($startDate) || $visitDate->gt($endDate)) {
            return back()
                ->with('error', 'Tanggal kunjungan harus dalam periode promo.')
                ->withInput();
        }

        // Generate order number
        $orderNumber = 'MK' . date('YmdHis') . Str::random(4);
